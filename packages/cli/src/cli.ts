...
import {
  DAEMON_READY_LOG_PATTERN,
  type DaemonLaunchResult,
  ensureTrailingNewline,
  findHealthcheckProcessId,
  getErrnoCode,
  ...
  MANAGED_DAEMON_VERIFY_INTERVAL_MS,
  MANAGED_DAEMON_VERIFY_TIMEOUT_MS,
  probeJson,
  spawnDetachedShell,
  type WaitForServerReadyConfig,
  waitForDetachedReadyFile,
  waitForServerReady,
  writeCurrentProcessLockFile,
...
const detectManagedDaemon = async () => {
  ...
  const listeningPid = findHealthcheckProcessId(HEALTHCHECK_URL);
...
const detectUnmanagedDaemon = async (): Promise<UnmanagedDaemon | null> => {
  ...
  return {
    pid: findHealthcheckProcessId(HEALTHCHECK_URL),
  };
};
...
const persistLaunchedOrListeningDaemonPid = (pid: number | null) => {
  ...
  return persistPidFileIfAlive(findHealthcheckProcessId(HEALTHCHECK_URL));
};
...
const launchDetachedServerWithSpawn = (
  ...
  launchDetachedProcessWithSpawn({
    ...
    logFile,
    logOffset,
    persistStartLock: persistDaemonStartLock,
    cleanupStartLock: cleanupDaemonStartLock,
  });
...
const launchDetachedServer = (): DaemonLaunchResult => {
...
const failedOperationMessage = (error: unknown, fallback: string) =>
...
const getDesktopExecutableCandidates = () => {
  ...
  if (process.platform === "darwin") {
    candidates.push(
      ...["Hive Desktop.app", "hive-desktop", "Hive.app", "hive-electron"].map(
        (fileName) => join(binaryDirectory, fileName)
      )
    );
...
const launchDesktopApplication = () => {
  ...
  try {
    ...
    return {
      ok: false,
      message: failedOperationMessage(error, "Failed to launch Hive desktop"),
    } as const;
  }
};
...
const tryAcquireDaemonStartLock = () => {
  ...
  ensurePidDirectory();
  return writeCurrentProcessLockFile(resolveDaemonStartLockFilePath());
};
...
class InstanceAddCommand extends Command {
  ...
  static override usage = Command.Usage({
    ...
    details:
      "Stores the instance URL in your local Hive config. Use --token-env to reference an environment variable; token values are not written to the profile.",
    examples: [
      [
        "Add company instance",
        "hive instance add company https://hive.example.com --use",
      ],
      [
        "Add separate web URL",
        "hive instance add company https://api.hive.example.com --web-url https://hive.example.com",
      ],
    ],
  });
...
class InstanceUseCommand extends Command {
  ...
  static override usage = Command.Usage({
    ...
    examples: [["Use company instance", "hive instance use company"]],
  });
...
class InstanceRemoveCommand extends Command {
  ...
  static override usage = Command.Usage({
    ...
    examples: [["Remove company instance", "hive instance remove company"]],
  });
...
class InstanceOpenCommand extends Command {
  ...
  static override usage = Command.Usage({
    ...
    examples: [
      ...
      ["Open company instance", "hive instance open company"],
    ],
  });
