...
describe("remote doctor Docker SSH E2E", () => {
  ...
  it("checks a disposable local SSH target end to end", async () => {
    ...
    tempRoot = mkdtempSync(join(tmpdir(), "hive-remote-worker-e2e-"));
    ...
    imageTag = `hive-remote-worker-doctor-e2e:${runId}`;
    containerName = `hive-remote-worker-doctor-e2e-${runId}`;
    ...
    assertCommandSucceeded(doctor, "hive remote doctor");

    expect(doctor.stdout).toContain("Hive remote doctor");
    ...
    expect(doctor.stdout).toContain(
      "workspace_root=/home/hive/.hive/workspaces"
    );
    expect(doctor.stdout).toContain("ok workspace_root exists");
    expect(doctor.stdout).toContain("ok workspace_root writable");
    expect(doctor.stdout).toContain(
      "Remote target is ready for Hive worker bootstrap."
    );
  });
});
