...
for pattern in "${PATTERNS[@]}"; do
    ...
    if git diff --cached --name-only \
        | grep -v -E "\.(lock|sum)$|package-lock\.json$|yarn\.lock$|bun\.lock$|routeTree\.gen\.ts$|skills-lock\.json$|^\.agents/skills/agent-browser/references/commands\.md$|^apps/hive_server_elixir/priv/resource_snapshots/" \
