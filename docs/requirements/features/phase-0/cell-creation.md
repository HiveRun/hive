...
- [x] Keep local include-copy support (`vendor/**`) but speed it up by copying static directory roots directly with ignore filters before file-level glob fallback (2026-02-14)
- [x] Keep include/ignore semantics explicit in the worktree copier so patterns remain predictable (`vendor/opencode/**` copies fully unless explicitly ignored) and avoid hidden auto-expansion side effects (2026-02-14)
...
- [x] Avoid duplicate provisioning UI by keeping chat terminal unmounted while a cell is `spawning`/`pending`; show a single waiting panel and attach chat only once status reaches `ready` (2026-02-15)
