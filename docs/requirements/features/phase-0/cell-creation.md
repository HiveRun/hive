...
- [x] Ignore stale `cell_provisioning_state` model overrides when a restorable OpenCode session already exists, so outdated creation-time overrides cannot block runtime startup before session model restoration (2026-02-19)
...
- [x] Decouple `send_initial_prompt` from the critical startup path by dispatching it in the background (with delayed-delivery warning logs) so `mark_ready` is never blocked by slow OpenCode prompt delivery, with regression coverage (2026-02-19)
