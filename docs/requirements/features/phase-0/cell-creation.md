...
- [x] Prevent deleted cells from reappearing after immediate page reload by introducing a persisted `deleting` cell status on delete-init, filtering `deleting` rows out of list/detail APIs, and evicting cell caches optimistically at mutation start so persisted query hydration cannot resurrect stale cells during in-flight cleanup (2026-02-17)
