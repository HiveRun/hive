...
> **Template Storage**: Templates are intentionally stored as files (`hive.config.jsonc` / `hive.config.json`; legacy `hive.config.ts` remains supported) rather than in the database. This architectural decision prioritizes version control, type safety, and developer experience over dynamic template management.
