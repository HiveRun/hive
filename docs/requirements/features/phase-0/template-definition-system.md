...
## Goal
Provide a flexible, type-safe system for defining cell templates that describe services, environments, and configuration.
...
- **Chose file-based**: Templates are defined in `hive.config.jsonc` / `hive.config.json`
- **Benefits**: 
...
- **Inline templates**: Keep templates inline within `hive.config.jsonc` for v1 simplicity (hive.config.json works too), with support for external template files in future versions.
...
- **Template variables**: Support `${env.VAR_NAME}` and `${cellDir}` templating in service configurations.

### Environment & Variables
- **Environment variables**: Per-service environment configuration with support for static values and template variables.
- **Variable resolution**: Resolve template variables during provisioning (ports, cell paths, workspace settings).
...
- **Teardown routines**: Template-level `teardown` array for cleanup commands when cell stops.
...
## Integration Points
- **Cell Creation/Provisioning**: Uses template definitions for workspace and service setup
