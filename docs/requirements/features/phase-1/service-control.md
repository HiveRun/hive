...
- Inject OpenCode custom tools (`hive_services`, `hive_service_logs`, `hive_setup_logs`) into each cell worktree so agents can pull recent service logs via the local Hive API using `HIVE_CELL_ID`.
