...
- On startup, Hive should detect constructs marked active, probe each recorded PID with `kill -0` (does not terminate the process) to see which services survived.
...
- Make it easy to copy the exact command/env that the supervisor uses (e.g., `hive services info <construct> <service>` prints the command) so users can run it manually if needed.
