...
## Goal
Enable cells to use Docker containers and Docker Compose for services alongside regular process-based services.

## Requirements

### Docker Container Support
- For `type: "docker"` services, Hive runs `docker run` with declared `image`, `command` (optional), `ports`, `env`, and `volumes`.
...
- Volume paths support `${cellDir}` templating so persistent data lives alongside the cell workspace.
- Docker services use deterministic container names (`hive-{cellId}-{serviceName}`) so stop commands can be autogenerated when omitted.

### Docker Compose Integration
- For `type: "compose"`, point to a Compose file (`composePath`) and optional service filter.
- Hive parses the Compose file, injects templated variables (including generated ports/env vars), and starts the selected services with the same status tracking/ready detection pipeline.
...
### Mixed Service Types
- Support hybrid cells with both Docker containers and regular processes
...
- **Cell Creation/Provisioning**: Handles Docker-specific provisioning steps
