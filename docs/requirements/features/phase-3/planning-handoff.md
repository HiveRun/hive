...
### Planning Agent Type
- **Planning cells**: Launch OpenCode in plan mode with limited toolset focused on analysis and documentation rather than code execution.
...
- **Persistence**: Store plans in SQLite + generate `PLAN.md` files in cell worktree for reference.
...
- **Revision handling**: When revisions are requested, return to planning cell to active state with user feedback incorporated into next prompt.

### Handoff to Implementation
- **Implementation cell creation**: Automatically create implementation cell from approved plan.
...
- **Cell Creation/Provisioning**: Creates implementation cells from approved plans
- **Persistence Layer**: Stores plan versions, feedback, and workflow state
- **Template Definition System**: Supports planning-specific cell templates
...
- **Conversion workflow**: On approval, create (or convert to) an implementation cell seeded with the approved plan context.
- **Context preservation**: Link the planning and implementation cells for full traceability.
- **Manual option**: Allow users to start a manual cell from the approved plan if they prefer to execute changes themselves.
...
### State Management
- **Status tracking**: Maintain clear cell states throughout the handoff process (planning → awaiting_approval → approved → implementation).
- **Rollback capability**: Support reverting from implementation back to planning if major issues are discovered.
- **Cross-cell linking**: Maintain bidirectional references between planning and implementation cells.

## Integration Points
- **Agent Orchestration Engine**: Manages both planning and implementation agent sessions
- **Persistence Layer**: Stores plans, versions, and cross-cell relationships
