...
## Cell Model (Rescoped)
- **Definition**: A cell bundles basic metadata, optional worktree, and optional agent session. Cells are instantiated from reusable templates defined in `hive.config.json`; each cell is owned and operated by the same single user who controls the workspace.
...
- [[features/phase-3/config-editor|Config editor]]: offer a UX for editing `hive.config.json` with validation, to be explored once the config API stabilizes.
