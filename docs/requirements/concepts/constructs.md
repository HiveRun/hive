...
- **Definition**: A construct bundles basic metadata, optional worktree, and optional agent session. Constructs are instantiated from reusable templates defined in `hive.config.ts`; each construct is owned and operated by the same single user who controls the workspace.
...
- Port allocation always probes the real host (not just internal state) so constructs spawned inside Hive avoid collisions with the live instance.
