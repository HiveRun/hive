...
export const DEPRECATED_CONFIG_FILENAMES = ["hive.config.jsonc"] as const;
...
export const findDeprecatedConfigPath = (directory: string): string | null => {
  for (const filename of DEPRECATED_CONFIG_FILENAMES) {
    const candidate = join(directory, filename);
    if (existsSync(candidate)) {
      return candidate;
    }
