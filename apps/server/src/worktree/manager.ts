...
export function createWorktreeManager(
  ...
  async function getIncludedPaths(
    ...
    try {
      ...
      const respectedMatches = await glob(expandedPatterns, baseOptions);
      for (const file of respectedMatches) {
        files.add(file);
      }

      const overrideMatches = await glob(expandedPatterns, {
        ...baseOptions,
        // tinyglobby type definitions don't expose this flag yet
        // eslint-disable-next-line @typescript-eslint/ban-ts-comment
        // @ts-expect-error -- upstream types missing gitignore option
        gitignore: false,
      });
      for (const file of overrideMatches) {
        files.add(file);
      }

      return Array.from(files, (file) => file.split(sep).join(POSIX_SEPARATOR));
    } catch (error: unknown) {
